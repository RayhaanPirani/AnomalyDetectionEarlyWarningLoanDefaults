# -*- coding: utf-8 -*-
"""
Created on Wed May 31 23:27:07 2023
...
DELOITTE_HACKATHON_LOANS_DATASET = 'deloitte_train.csv'

dh_loan_df = pd.read_csv(DELOITTE_HACKATHON_LOANS_DATASET)

# Dropping irrelevant columns
irrelevant_cols = ['ID', 'Batch Enrolled', 'Payment Plan']
dh_loan_df = dh_loan_df.drop(columns=irrelevant_cols)

# Mapping ordinal categorical column values to numbers
category_cols = ['Grade', 'Sub Grade', 'Verification Status',
                 'Initial List Status', 'Application Type']
for category in category_cols:
    dh_loan_df[category] = dh_loan_df[category].astype('category').cat.codes

dh_loan_df = dh_loan_df.join(pd.get_dummies(dh_loan_df['Employment Duration'])).drop(
    ['Employment Duration', 'MORTGAGE'], axis=1)

# Condensing Loan Title categories

dh_loan_df['Loan Title'] = dh_loan_df['Loan Title'].str.upper()

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('CONSO') | \
    dh_loan_df['Loan Title'].str.contains('REFI') | \
    dh_loan_df['Loan Title'].str.contains('PAYOFF') | \
    dh_loan_df['Loan Title'].str.contains('PAY OFF') | \
    dh_loan_df['Loan Title'].str.contains('RELIEF') | \
    dh_loan_df['Loan Title'].str.contains('FREE') | \
    dh_loan_df['Loan Title'].str.contains('DEBT') | \
    dh_loan_df['Loan Title'].str.contains('PAYDOWN') | \
    dh_loan_df['Loan Title'].str.contains('AHEAD'),
        'CONSOLIDATION', dh_loan_df['Loan Title'])

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('CREDIT') | \
    dh_loan_df['Loan Title'].str.contains('CC') | \
    dh_loan_df['Loan Title'].str.contains('CARD'),
        'CREDIT CARD', dh_loan_df['Loan Title'])

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('HOME') | \
    dh_loan_df['Loan Title'].str.contains('HOUSE') | \
    dh_loan_df['Loan Title'].str.contains('ROOM') | \
    dh_loan_df['Loan Title'].str.contains('POOL') | \
    dh_loan_df['Loan Title'].str.contains('MOVING'),
        'HOME', dh_loan_df['Loan Title'])

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('MED'),
    'MEDICAL', dh_loan_df['Loan Title'])

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('CAR '),
    'CAR', dh_loan_df['Loan Title'])

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('MAJOR') | \
    dh_loan_df['Loan Title'].str.contains('VACATION') | \
    dh_loan_df['Loan Title'].str.contains('WEDDING') | \
    dh_loan_df['Loan Title'].str.contains('PERSONAL') | \
    dh_loan_df['Loan Title'].str.contains('GREEN'),
        'PERSONAL', dh_loan_df['Loan Title'])

dh_loan_df['Loan Title'] = np.where(dh_loan_df['Loan Title'].str.contains('LEND') | \
    dh_loan_df['Loan Title'].str.contains('LOAN'),
        'PERSONAL', dh_loan_df['Loan Title'])

dh_loan_df = dh_loan_df.join(pd.get_dummies(dh_loan_df['Loan Title'])).drop(
    ['Loan Title', 'OTHER'], axis=1)

X = dh_loan_df.drop(columns=['Loan Status'])
y = dh_loan_df['Loan Status']
