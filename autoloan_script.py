# -*- coding: utf-8 -*-
"""
Created on Wed Mar 15 14:17:01 2023
...
AUTOMOBILE_LOANS_DATASET = 'train.csv'

autoloan_df = pd.read_csv(AUTOMOBILE_LOANS_DATASET)

# Converting date columns to age columns
...
def date_to_months_since(x):
    if str(x) == 'nan':
        return None
    now = datetime.today()
    then = datetime.strptime(x, '%d-%m-%y')
    if now <= then:
        then = then.replace(year=then.year - 100)
    return round((now - then).days/30)

autoloan_df['MonthsSinceDisbursalDate'] = \
    autoloan_df['DisbursalDate'].apply(date_to_months_since)

autoloan_df['Age'] = \
    autoloan_df['Date.of.Birth'].apply(date_to_months_since).apply(
        lambda x: math.floor(x / 12))

autoloan_df = autoloan_df.drop(columns=date_cols)

def yrs_mon_to_months(x):
    if str(x) == 'nan':
        return None
    yrs_mon = [int(y) for y in
               str(x).replace('yrs', '').replace('mon', '').split()]
    return yrs_mon[0] * 12 + yrs_mon[1]

autoloan_df['AVERAGE.ACCT.AGE'] = \
    autoloan_df['AVERAGE.ACCT.AGE'].apply(yrs_mon_to_months)

autoloan_df['CREDIT.HISTORY.LENGTH'] = \
    autoloan_df['CREDIT.HISTORY.LENGTH'].apply(yrs_mon_to_months)

category_cols = ['Employment.Type', 'PERFORM_CNS.SCORE.DESCRIPTION']
...
for category in category_cols:
    ...
    autoloan_df[category] = autoloan_df[category].map(category_score)

autoloan_df = autoloan_df.dropna()
autoloan_df.reset_index(inplace=True, drop=True)

autoloan_df['loan_to_asset_ratio'] = autoloan_df['disbursed_amount'] / autoloan_df['asset_cost']
autoloan_df['total_accounts'] = autoloan_df['PRI.NO.OF.ACCTS'] + autoloan_df['SEC.NO.OF.ACCTS']
autoloan_df['pri_inactive_accts'] = autoloan_df['PRI.NO.OF.ACCTS'] - autoloan_df['PRI.ACTIVE.ACCTS']
autoloan_df['sec_inactive_accts'] = autoloan_df['SEC.NO.OF.ACCTS'] - autoloan_df['SEC.ACTIVE.ACCTS']
autoloan_df['total_inactive_accts'] = autoloan_df['pri_inactive_accts'] + autoloan_df['sec_inactive_accts']
autoloan_df['total_active_accts'] = autoloan_df['PRI.ACTIVE.ACCTS'] + autoloan_df['SEC.ACTIVE.ACCTS']
autoloan_df['total_curr_bal'] = autoloan_df['PRI.CURRENT.BALANCE'] + autoloan_df['SEC.CURRENT.BALANCE']
autoloan_df['total_sanctioned_amt'] = autoloan_df['PRI.SANCTIONED.AMOUNT'] + autoloan_df['SEC.SANCTIONED.AMOUNT']
autoloan_df['total_disbursed_amt'] = autoloan_df['PRI.DISBURSED.AMOUNT'] + autoloan_df['SEC.DISBURSED.AMOUNT']
autoloan_df['total_instal_amt'] = autoloan_df['PRIMARY.INSTAL.AMT'] + autoloan_df['SEC.INSTAL.AMT']
autoloan_df['pri_loan_proportions'] = autoloan_df['PRI.DISBURSED.AMOUNT'] / (autoloan_df['PRIMARY.INSTAL.AMT'] + 1)
autoloan_df['sec_loan_proportions'] = autoloan_df['SEC.DISBURSED.AMOUNT'] / (autoloan_df['SEC.INSTAL.AMT'] + 1)
autoloan_df['active_to_inactive_acct_ratio'] = autoloan_df['total_active_accts'] / (autoloan_df['total_inactive_accts'] + 1)

quantitative_cols = ['PERFORM_CNS.SCORE','PRI.NO.OF.ACCTS','PRI.ACTIVE.ACCTS',
    'PRI.OVERDUE.ACCTS','PRI.CURRENT.BALANCE','PRI.SANCTIONED.AMOUNT',
    'PRI.DISBURSED.AMOUNT','SEC.NO.OF.ACCTS','SEC.ACTIVE.ACCTS',
    'SEC.OVERDUE.ACCTS','SEC.CURRENT.BALANCE','SEC.SANCTIONED.AMOUNT',
    'SEC.DISBURSED.AMOUNT','PRIMARY.INSTAL.AMT','SEC.INSTAL.AMT',
    'NEW.ACCTS.IN.LAST.SIX.MONTHS','DELINQUENT.ACCTS.IN.LAST.SIX.MONTHS',
    'loan_to_asset_ratio','total_accounts','pri_inactive_accts',
    'sec_inactive_accts','total_inactive_accts','total_active_accts',
    'total_curr_bal','total_sanctioned_amt','total_disbursed_amt',
    'total_instal_amt','pri_loan_proportions','sec_loan_proportions',
    'active_to_inactive_acct_ratio']
autoloan_quant_df = autoloan_df[quantitative_cols]
autoloan_df = autoloan_df[(autoloan_quant_df == 0).sum(axis=1) / len(
    autoloan_quant_df.columns) > 0.75]
autoloan_df.reset_index(inplace=True, drop=True)

irrelevant_cols = ['UniqueID','branch_id','supplier_id','manufacturer_id',
    'Current_pincode_ID','State_ID','Employee_code_ID','MobileNo_Avl_Flag']
autoloan_df = autoloan_df.drop(columns=irrelevant_cols)

X = autoloan_df.drop(columns=['loan_default'])
y = autoloan_df['loan_default']
